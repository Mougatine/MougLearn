...
class MySoftMarginSVM:
    ...
    def __compute_gram_matrix(self, x, y):
        ...
        for i in range(self.N // 2):
            for j in range(self.N // 2):
                ...
                self.__gram_mat[j, i] = y[j] * y[i] * np.dot(x[j, :], x[i, :])
    ...
    def train(self, x, y):
        ...
        sv = np.where((self.__alphas > 1e-5) & (self.__alphas <= self.C))[0]
        not_sv = np.where((self.__alphas < 1e-5) | (self.__alphas > self.C))[0]
